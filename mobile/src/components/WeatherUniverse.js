...
// ─── Scene constants ──────────────────────────────────────────────────────────
const SCENE_H    = 250;
const SKY_H      = 160;      // horizon level (flat reference)
const CHAR_W     = 52;
const CHAR_H     = 120;
const CHAR_Y     = 68;       // head Y; feet at 68+120=188
const WALK_DUR   = 7000;

// World-curve: horizon dips at edges, rises at center — the AC globe effect
const C_PEAK = SKY_H - 10;   // horizon Y at screen centre (highest = closest)
const C_EDGE = SKY_H + 22;   // horizon Y at screen edges  (lowest  = furthest)

// Horizon Y at any X position (quadratic bezier formula)
function hz(x, w) {
  const t = x / w;
  return (1-t)*(1-t)*C_EDGE + 2*t*(1-t)*C_PEAK + t*t*C_EDGE;
}

// ─── Outfit items ─────────────────────────────────────────────────────────────
const ITEMS = {
  hot:      { top:['Tank Top',    '#FF8C00'], bot:['Shorts',      '#5BC8F5'], shoe:['Sandals',      '#C8A850'] },
  warm:     { top:['T-Shirt',     '#5DADE2'], bot:['Shorts',      '#C8A96E'], shoe:['Sneakers',     '#D8D8D8'] },
  mild:     { top:['Long Sleeve', '#5B9BD5'], bot:['Jeans',       '#3A5FCD'], shoe:['Sneakers',     '#D8D8D8'] },
  cool:     { top:['Sweater',     '#CC6633'], bot:['Jeans',       '#3A5FCD'], shoe:['Sneakers',     '#D8D8D8'] },
...
const GROUND_COL = {
  ...
  cold:'#14532D', freezing:'#B0C4CE', rain:'#334155', snow:'#E2EBF0',
...
// ─── AC-style tree (single large crown + perimeter bumps) ─────────────────────
function ACTree({ cx, baseY, sc=1, isSnow=false }) {
  const r=22*sc, tw=11*sc, th=20*sc;
  const cy=baseY - th - r*.82;
  ...
  const bumps=[[-40,.88],[20,.88],[75,.84],[125,.87],[165,.84],[-100,.86]];
  return (
    <G>
      <Rect x={cx-tw/2} y={baseY-th} width={tw} height={th} fill={trunk} stroke={trunkD} strokeWidth={1.5} rx={tw/2}/>
      {bumps.map(([a,f],i)=>{
        const rad=a*Math.PI/180;
        return <Circle key={i} cx={cx+Math.cos(rad)*r*f} cy={cy+Math.sin(rad)*r*f} r={r*.42} fill={green} stroke={outline} strokeWidth={1}/>;
      })}
...
function PineTree({ cx, baseY, sc=1, isSnow=false }) {
  ...
  return (
    <G>
      ...
      <Rect x={cx-3*sc} y={baseY-5*sc} width={6*sc} height={5*sc} fill="#8B5E3C"/>
    </G>
  );
}

// ─── Curved ground + path + details (all in SVG coords) ──────────────────────
function GroundLayer({ w, groundCol, pathCol, outfitType }) {
  const d=`M 0 ${C_EDGE} Q ${w/2} ${C_PEAK} ${w} ${C_EDGE} L ${w} ${SCENE_H+10} L 0 ${SCENE_H+10} Z`;
  // Lighter top edge stripe for depth
  const d2=`M 0 ${C_EDGE} Q ${w/2} ${C_PEAK} ${w} ${C_EDGE} Q ${w/2} ${C_PEAK+5} 0 ${C_EDGE+2} Z`;
  // Perspective path converging to horizon center
  const pw=58;
  const pathD=`M ${w/2-pw/2} ${SCENE_H+5} L ${w/2-4} ${C_PEAK+4} L ${w/2+4} ${C_PEAK+4} L ${w/2+pw/2} ${SCENE_H+5} Z`;

  const isWarm=['mild','cool','warm','athletic','hot'].includes(outfitType);
  const isRain =outfitType==='rain';
  const isSnow =['snow','freezing'].includes(outfitType);

  return (
    <>
      <Path d={d}    fill={groundCol}/>
      <Path d={d2}   fill="rgba(255,255,255,0.12)"/>
      <Path d={pathD} fill={pathCol} opacity={.32}/>

      {/* Foreground grass blades at extreme edges for depth */}
      {isWarm&&[{x:w*.04},{x:w*.96}].map((g,i)=>(
        <G key={i}>
          {[-10,-4,2,8,14].map((dx,j)=>(
            <Path key={j} d={`M ${g.x+dx} ${SCENE_H} Q ${g.x+dx+3} ${SCENE_H-18} ${g.x+dx+6} ${SCENE_H}`}
      ...
      {/* Flowers */}
      {isWarm&&[{x:w*.14,c:'#FCD34D'},{x:w*.38,c:'#F9A8D4'},{x:w*.62,c:'#86EFAC'},{x:w*.82,c:'#FCD34D'}].map((f,i)=>(
        <G key={i}>
          <Circle cx={f.x} cy={SCENE_H-20} r={5} fill={f.c} stroke="rgba(0,0,0,.08)" strokeWidth={1}/>
          <Rect x={f.x-1} y={SCENE_H-15} width={2.5} height={14} fill="#276B2E"/>
        </G>
      ))}

      {/* Rain puddles */}
      {isRain&&[w*.16,w*.48,w*.76].map((x,i)=>(
        <Ellipse key={i} cx={x} cy={SCENE_H-10} rx={18} ry={6} fill="rgba(148,163,184,.4)"/>
      ))}

      {/* Snow drifts */}
      {isSnow&&[{x:w*.12,r:22},{x:w*.42,r:16},{x:w*.68,r:20},{x:w*.88,r:18}].map((d,i)=>(
        <Ellipse key={i} cx={d.x} cy={SCENE_H-8} rx={d.r} ry={9} fill="rgba(255,255,255,.75)"/>
      ))}
    </>
  );
}

// ─── Rolling hills + AC trees (trees follow curved horizon) ───────────────────
function GenericBG({ w, outfitType }) {
  ...
  const fh=SKY_H*.56, nh=SKY_H*.74;
  // Hills end at the curved horizon per x position
  const far=`M 0 ${fh} C ${w*.12} ${fh*.7},${w*.23} ${fh*.54},${w*.34} ${fh*.74} C ${w*.46} ${fh*.95},${w*.57} ${fh*.5},${w*.68} ${fh*.66} C ${w*.79} ${fh*.8},${w*.91} ${fh*.55},${w+10} ${fh*.7} L ${w+10} ${hz(w,w)+4} L 0 ${hz(0,w)+4} Z`;
  const near=`M 0 ${nh} C ${w*.1} ${nh*.82},${w*.26} ${nh*.9},${w*.39} ${nh*.84} C ${w*.53} ${nh*.78},${w*.63} ${nh*.88},${w*.77} ${nh*.79} C ${w*.89} ${nh*.73},${w*.97} ${nh*.9},${w+10} ${nh*.86} L ${w+10} ${hz(w,w)+2} L 0 ${hz(0,w)+2} Z`;
  // Trees: baseY follows the curved horizon so they appear on the globe
  const trees=[{x:w*.07,sc:1.3},{x:w*.19,sc:1.65},{x:w*.81,sc:1.55},{x:w*.92,sc:1.25}];
  return (
    <>
      <Path d={far}  fill={hFar}/>
      <Path d={near} fill={hNear}/>
      {trees.map((t,i)=>(
        <ACTree key={i} cx={t.x} baseY={hz(t.x,w)+6} sc={t.sc} isSnow={isSnow}/>
      ))}
    </>
  );
}
...
function MountainBG({ w, outfitType }) {
  ...
  const m1=`M ${-w*.05} ${SKY_H} Q ${w*.17} ${SKY_H*.08} ${w*.38} ${SKY_H} Z`;
  const m2=`M ${w*.22}  ${SKY_H} Q ${w*.46} ${SKY_H*.02} ${w*.70} ${SKY_H} Z`;
  const m3=`M ${w*.50}  ${SKY_H} Q ${w*.73} ${SKY_H*.14} ${w*.96} ${SKY_H} Z`;
  const s1=`M ${w*.14} ${SKY_H*.26} Q ${w*.17} ${SKY_H*.08} ${w*.2}  ${SKY_H*.26} Z`;
  const s2=`M ${w*.42} ${SKY_H*.2}  Q ${w*.46} ${SKY_H*.02} ${w*.5}  ${SKY_H*.2}  Z`;
  const s3=`M ${w*.69} ${SKY_H*.31} Q ${w*.73} ${SKY_H*.14} ${w*.77} ${SKY_H*.31} Z`;
  const pines=[{x:w*.08,sc:.8},{x:w*.31,sc:.7},{x:w*.72,sc:.75},{x:w*.9,sc:.85}];
  return (
    <>
      ...
      {pines.map((p,i)=><PineTree key={i} cx={p.x} baseY={hz(p.x,w)+6} sc={p.sc} isSnow={isSnow}/>)}
    </>
  );
}
...
function BeachBG({ w }) {
  const oy=SKY_H*.44;
  const ocean=`M 0 ${oy} Q ${w*.3} ${oy-9} ${w*.6} ${oy} Q ${w*.8} ${oy+7} ${w} ${oy} L ${w} ${SKY_H+10} L 0 ${SKY_H+10} Z`;
  const wave1=`M 0 ${oy+13} Q ${w*.15} ${oy+7} ${w*.3} ${oy+13} Q ${w*.45} ${oy+19} ${w*.6} ${oy+13}`;
  const angles=[-140,-110,-80,-50,-20];
  const p1={x:w*.13,y:hz(w*.13,w)-60}, p2={x:w*.86,y:hz(w*.86,w)-52};
  return (
    <>
      <Path d={ocean} fill="#1565C0" opacity={.48}/>
      <Path d={wave1} fill="none" stroke="rgba(255,255,255,.45)" strokeWidth={2}/>
      <Path d={`M ${w*.10} ${SCENE_H} Q ${w*.11} ${p1.y+30} ${p1.x} ${p1.y}`} fill="none" stroke="#8B5E3C" strokeWidth={7} strokeLinecap="round"/>
      <Path d={`M ${w*.89} ${SCENE_H} Q ${w*.90} ${p2.y+26} ${p2.x} ${p2.y}`} fill="none" stroke="#8B5E3C" strokeWidth={6} strokeLinecap="round"/>
      {angles.map((a,i)=>{
        const rad=a*Math.PI/180, len=27+i*1.5;
        return <Path key={i} d={`M ${p1.x} ${p1.y} Q ${p1.x+Math.cos(rad)*len*.5} ${p1.y+Math.sin(rad)*len*.5} ${p1.x+Math.cos(rad)*len} ${p1.y+Math.sin(rad)*len}`} fill="none" stroke="#276B2E" strokeWidth={3} strokeLinecap="round"/>;
      })}
      {angles.map((a,i)=>{
        const rad=a*Math.PI/180, len=23+i*1.3;
        return <Path key={i} d={`M ${p2.x} ${p2.y} Q ${p2.x+Math.cos(rad)*len*.5} ${p2.y+Math.sin(rad)*len*.5} ${p2.x+Math.cos(rad)*len} ${p2.y+Math.sin(rad)*len}`} fill="none" stroke="#1A5C22" strokeWidth={2.5} strokeLinecap="round"/>;
      })}
    </>
  );
}
...
function CityBG({ w, isNight }) {
  const bc=isNight?'#0F172A':'#2D3748', sc=isNight?'#1E293B':'#3D4F62';
  return (
    <>
      {BLDGS.map((b,i)=>(
        <G key={i}>
          <Rect x={b.x} y={SKY_H-b.h} width={b.w} height={b.h} fill={bc} stroke={sc} strokeWidth={1}/>
          {isNight&&b.wins.map(([row,col],j)=>(
            <Rect key={j} x={b.x+3+col*8} y={SKY_H-b.h+8+row*13} width={5} height={6} fill="#FEF08A" opacity={.88} rx={1}/>
          ))}
        </G>
      ))}
      {isNight&&[w*.22,w*.5,w*.76].map((lx,i)=>(
        <G key={i}>
          <Rect x={lx} y={SKY_H-30} width={3} height={30} fill="#4B5563"/>
          <Ellipse cx={lx+1.5} cy={SKY_H-32} rx={11} ry={5} fill="rgba(254,240,138,.18)"/>
          <Rect x={lx-2} y={SKY_H-34} width={7} height={4} fill="#FEF08A" rx={2}/>
        </G>
      ))}
    </>
  );
...
function Cloud({ startX, y, dur, sceneW }) {
  ...
  useEffect(()=>{ Animated.loop(Animated.sequence([Animated.timing(x,{toValue:sceneW+110,duration:dur,useNativeDriver:true}),Animated.timing(x,{toValue:-110,duration:0,useNativeDriver:true})])).start(); },[]);
  return (
    <Animated.View style={{position:'absolute',top:y,transform:[{translateX:x}]}}>
      <View style={{flexDirection:'row',alignItems:'flex-end'}}>
        ...
        <View style={{width:44,height:26,borderRadius:14,backgroundColor:'rgba(255,255,255,.93)'}}/>
...
function Rain({ sceneW }) {
  ...
  return (
    <View style={[StyleSheet.absoluteFill,{height:SKY_H}]} pointerEvents="none">
      {anims.map((a,i)=><Animated.View key={i} style={{position:'absolute',left:(sceneW/n)*i+2,top:0,width:1.5,height:14,backgroundColor:'rgba(148,163,184,.65)',borderRadius:1,transform:[{translateY:a.interpolate({inputRange:[0,1],outputRange:[0,SKY_H]})}],opacity:a.interpolate({inputRange:[0,.8,1],outputRange:[.8,.8,0]})}}/>)}
    </View>
  );
}
...
function Snow({ sceneW }) {
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      {anims.map((a,i)=><Animated.Text key={i} style={{position:'absolute',left:xs[i],fontSize:12,color:'rgba(255,255,255,.92)',transform:[{translateY:a.interpolate({inputRange:[0,1],outputRange:[0,SKY_H+10]})},{translateX:a.interpolate({inputRange:[0,.5,1],outputRange:[0,9,0]})}],opacity:a.interpolate({inputRange:[0,.85,1],outputRange:[1,1,0]})}}>✦</Animated.Text>)}
    </View>
  );
}
...
function Stars() {
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      {[{x:22,y:8},{x:68,y:5},{x:115,y:17},{x:158,y:7},{x:205,y:13},{x:252,y:6},{x:295,y:20}].map((p,i)=>(
...
function Celestial({ outfitType, timeBlock, topSkyCol }) {
  ...
  return (
    <Animated.View style={{position:'absolute',top,left,alignItems:'center',justifyContent:'center',transform:[{scale:pulse}]}}>
      <View style={{position:'absolute',width:58,height:58,borderRadius:29,backgroundColor:glow}}/>
...
function Char({ items, legAnim, kickAnim, gender }) {
  ...
  return (
    <Animated.View style={{alignItems:'center',transform:[{translateY:bob}]}}>
      {/* Hair */}
      {gender==='womens'&&<View style={{width:48,height:18,borderTopLeftRadius:24,borderTopRightRadius:24,borderBottomLeftRadius:6,borderBottomRightRadius:6,backgroundColor:hairCol}}/>}
      {gender==='mens'  &&<View style={{width:32,height:12,borderTopLeftRadius:16,borderTopRightRadius:16,backgroundColor:hairCol}}/>}
      {gender==='neutral'&&<View style={{width:28,height:9,borderTopLeftRadius:14,borderTopRightRadius:14,backgroundColor:hairCol}}/>}

      {/* Head */}
      <View style={{width:38,height:36,borderRadius:19,backgroundColor:skin,borderWidth:1.5,borderColor:'rgba(0,0,0,.1)',alignItems:'center',justifyContent:'center',marginTop:gender==='womens'?-10:gender==='mens'?-6:-5}}>
        {gender==='womens'&&<View style={{flexDirection:'row',gap:10,marginBottom:2}}>{[0,1].map(i=><View key={i} style={{width:9,height:3,borderRadius:2,backgroundColor:'rgba(0,0,0,.3)',transform:[{rotate:i===0?'-6deg':'6deg'}]}}/>)}</View>}
        <View style={{flexDirection:'row',gap:gender==='womens'?8:7,marginTop:gender==='womens'?0:5}}>
          {[0,1].map(i=>(
            <View key={i} style={{width:6,height:6,borderRadius:3,backgroundColor:'#1F2937',alignItems:'flex-end',justifyContent:'flex-start',padding:.5}}>
              <View style={{width:2,height:2,borderRadius:1,backgroundColor:'white'}}/>
            </View>
          ))}
        </View>
        {gender==='womens'&&<View style={{flexDirection:'row',position:'absolute',bottom:7,left:3,right:3,justifyContent:'space-between'}}>{[0,1].map(i=><View key={i} style={{width:7,height:3,borderRadius:3,backgroundColor:'#FCA5A5',opacity:.65}}/>)}</View>}
        <View style={{width:11,height:5,borderBottomLeftRadius:6,borderBottomRightRadius:6,borderBottomWidth:2,borderLeftWidth:1.5,borderRightWidth:1.5,borderColor:'rgba(0,0,0,.25)',marginTop:gender==='womens'?2:3}}/>
      </View>
...
export default function WeatherUniverse({
  ...
  const ROCK_X   = sceneW * 0.56;
  const ROCK_Y   = CHAR_Y + CHAR_H - 8;  // same level as feet
  ...
  return (
    <View style={s.wrap}>
      <View style={s.scene} onLayout={onLayout}>
        ...
        {/* Gradient sky — full scene height */}
        <LinearGradient colors={skyGrad} style={StyleSheet.absoluteFill} start={{x:0,y:0}} end={{x:0,y:1}}/>

        {/* Full-height SVG: ground curve + background art (painter's order) */}
        <View style={{position:'absolute',top:0,left:0,width:sceneW,height:SCENE_H}} pointerEvents="none">
          <Svg width={sceneW} height={SCENE_H}>
            {/* 1. Ground (curved horizon — the AC globe effect) */}
            <GroundLayer w={sceneW} groundCol={groundCol} pathCol={pathCol} outfitType={outfitType}/>

            {/* 2. Background silhouettes on top of ground */}
        ...
        {!isStorm&&!isNight&&(
          <>
            ...
            <Cloud startX={-90} y={34} dur={28000} sceneW={sceneW}/>
          </>
        )}
        {outfitType==='rain'&&<Rain sceneW={sceneW}/>}
        {['snow','freezing'].includes(outfitType)&&<Snow sceneW={sceneW}/>}

        {/* Rock (sits on ground at foot level) */}
        <Animated.View style={{position:'absolute',top:ROCK_Y,left:ROCK_X,transform:[{translateX:rockFly}]}}>
          <View style={{width:13,height:10,borderRadius:5,backgroundColor:'rgba(0,0,0,.2)',borderWidth:1,borderColor:'rgba(0,0,0,.08)'}}/>
        </Animated.View>

        {/* Character — walks on the curved ground */}
        ...
        {/* Badges */}
        {eventType!=='casual'&&<View style={[s.badge,{right:10}]}><Text style={s.badgeTxt}>{eventType[0].toUpperCase()+eventType.slice(1)}</Text></View>}
